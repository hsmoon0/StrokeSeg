...
data_x_train = data_cropped_train[:,:,:,2]
...
data_x_test = data_cropped_test[:,:,:,2]
...
data_x_train_blur = np.empty([len(data_x_train),448,512,1])
for i in range(data_x_train.shape[0]): 
  blur_train = cv2.GaussianBlur(data_x_train[i,:,:,0], (3,3), 0)
  data_x_train_blur[i, :, :, 0] = blur_train

data_x_test_blur = np.empty([len(data_x_test),448,512,1])
for i in range(data_x_test.shape[0]): 
  blur_test = cv2.GaussianBlur(data_x_test[i,:,:,0], (3,3), 0)
  data_x_test_blur[i, :, :, 0] = blur_test
...
def cnn(fliter_num,kernel_size):
    ...
    np.save('/home/alex/Stroke/results/train_loss_adc_blur_3.npy',train_loss)
    train_acc = history.history['dice_metric']
    np.save('/home/alex/Stroke/results/train_acc_adc_blur_3.npy',train_acc)
    val_loss = history.history['val_loss']
    np.save('/home/alex/Stroke/results/val_loss_adc_blur_3.npy',val_loss)
    val_acc = history.history['val_dice_metric']
    np.save('/home/alex/Stroke/results/val_acc_adc_blur_3.npy',val_acc)

    model.save('/home/alex/Stroke/results/model_adc_blur_3.h5')
